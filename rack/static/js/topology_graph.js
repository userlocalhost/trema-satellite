...
Screen.prototype = function() {
	...
	function drawScaleLine(panel) {
		panel.add(pv.Panel)
			...
			.add(pv.Bar)
				...
				.width( FREQUENCY_SCALE_WIDTH )
				.height(3)
				.top(function() { return ( this.index * (FREQUENCY_SCALE_HEIGHT / FREQUENCY_SCALE_LEVEL) ) })
	...
	return {
		...
		draw: function() {
			initBaseInfo(this.width, this.height, this.view_elem);
			initPortInfo();
