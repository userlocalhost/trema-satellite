...
class GraphController
	...
	def switch_ready dpid
		...
		Intervals.new PORT_STATS_INTERVAL do
			print "(switch_ready) send_message #{dpid}, PortStatsRequest.new\n"
	...
	def send_flow_mod_add dpid, options
		if options[:match] != nil then
			...
			if ! Graph::Route.may_append_entry entry then
				info "[send_flow_mod_add (override)] 結合できない"
				...
				info "[send_flow_mod_add (override)] 結合できる！"
				# unknown 経路同士の結合チェック
			end
	...
	def packet_in dpid, msg
		info "[GraphController] (packet_in) dpid: #{dpid}"
		...
		if msg.eth_type == 0xffff
			...
			info "[GraphController] datapath (nid:#{current_port.node_id}).set_neighbor #{from_portobj.node_id}"
			info "[GraphController] datapath (nid:#{from_portobj.node_id}).set_neighbor #{current_port.node_id}"
			...
			info "[GraphController] (discover_response) (#{dpid}:#{msg.in_port}) dpid:#{from_dpid}, port:#{from_port}"
		else
			if ! Graph::Host.isin? msg.macsa
				...
				info "[GraphController] host (nid:#{port.node_id}).set_neighbor #{host.node_id}"
	...
	def stats_reply dpid, msg
		msg.stats.each do |each|
			case each
			when PortStatsReply
				info "[stats_reply] (PortStatsReply)"
			...
			when FlowStatsReply
				...
				info "[stats_reply] (FlowStatsReply) entry: #{entry}"
	...
	def create_discover_response_frame dpid, portnum
		info "[create_discover_response_frame] dpid:#{dpid}, portnum:#{portnum}"
	...
	def send_discover_request dpid
		...
		msg = PacketIn.new :datapath_id => 0x1, :match => match, 
	...
	def send_discover_response dpid, portnum
		action = Actions::SendOutPort.new :port_number => portnum
