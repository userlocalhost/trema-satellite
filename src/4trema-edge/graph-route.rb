...
module Graph
	class Route
		...
		def may_append_entry target
			...
			print "[Route] (may_append_entry) Is target appendable first? #{target.is_appendable? @entries.first}\n"
			print "[Route] (may_append_entry) Is target appendable last? #{@entries.last.is_appendable? target}\n"
		...
		def check_termination
			@@unknown.each do |each|
				print "[Route] (check_termination) "
				each.entries.each do |entry|
					print "(#{entry.match.in_port}:#{entry.dpid}:#{entry.get_output_portnum}), "
				end
				print "\n"

				print "[Route] (check_termination) is_from_host?: #{each.entries.first.is_from_host?}\n"
				print "[Route] (check_termination) is_to_host?: #{each.entries.last.is_to_host?}\n"
		...
		def self.dump_unknown
			...
			@@unknown.each do |each|
				each.entries.each do |entry|
					print "(#{entry.match.in_port}:#{entry.dpid}:#{entry.get_output_portnum}), " if entry.match
				end
				print "\n"
			end
