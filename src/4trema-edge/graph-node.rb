...
module Graph
	...
	class Port
		...
		def set_neighbor node_id
			print "[Graph::Port] (set_neighbor) dpid:#{@dpid}, portnum:#{portnum} => #{node_id}\n"
			@neighbor = node_id
		...
		def isin_db?
			...
			print "[Port] (isin_db?) (#{@dpid}:#{@portnum}) query: #{query} (#{query.num_rows})\n"
		...
		def self.get dpid, portnum
			...
			if ! port || (! port.instance_of? self) then
				print "[Port.get] create a new Port (#{dpid}:#{portnum}) [#{port}]\n"
	...
	class Host
		...
		def initialize mac, ip, neighbor
			...
			print "[Graph::Host] (initialize) @node_id: #{@node_id}\n"
			#@node_id = 0
		end
		...
		def isin_db?
			...
			print "[Graph::Host] (isin_db?) sql: #{sql}\n"
		...
		def self.add_node
			...
			print "[Graph::Host (add_node)] insert_id: #{insert_id}\n"
